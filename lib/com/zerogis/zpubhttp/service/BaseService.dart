...
class BaseService
{
   ...
   * 获取窗口
   */
  static FuctionStateBase getBaseSceneState(HttpListener listener)
  {
    if (listener is FuctionStateBase)
    {
      return listener as FuctionStateBase;
    }
    return null;
  }
   ...
   * 显示网络进度栏及关闭软键盘
   */
  static void showProgressBar(HttpListener listener, {String text})
  {
    FuctionStateBase state = getBaseSceneState(listener);
    state.setState(()
    {
      state.showProgressBar(text: text);
    });
  }
  ...
  {
    final FuctionStateBase activity = getBaseSceneState(listener);
